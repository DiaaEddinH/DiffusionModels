...
class TestBaseSampler:
    ...
    def test_init_sets_attributes_from_model(self, sampler, dummy_model, dummy_schedule):
...
class TestBuildSchedule:
    ...
    def test_timesteps_from_one_to_eps(self, real_sampler, schedule_type):
        timesteps, _, _, _ = real_sampler.build_schedule(30, schedule_type=schedule_type)
    ...
    @pytest.mark.parametrize("schedule_type", ["uniform", "log", "karras"])
    def test_timesteps_monotontest_step_sizes_nonnegative(self, real_sampler, schedule_type):
        _, _, step_size, step_size_sqrt = real_sampler.build_schedule(30, schedule_type=schedule_type)
...
class TestSample:
    ...
    def test_final_step_uses_zero_diffusion_when_t_below_eps(self, sampler, dummy_model, monkeypatch):
...
def test_ot_sampler_history_and_timesteps_and_no_grad_and_labels():
    ...
    model = DummyModel(device="cpu", schedule=schedule, output_scale=0.0)  # v(x,t)=0 => x stays constant
