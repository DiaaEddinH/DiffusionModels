...
class TestBuildOptimizer:
    ...
    def test_optimizer_tracks_model_parameters(self):
        model = TinyModel()
        optimizer = build_optimizer(model, ComponentConfig(name="adam", params={"lr": 1e-3}))
...
class TestBuildLrScheduler:
    def test_returns_none_when_config_is_none(self):
        model = TinyModel()
        optimizer = build_optimizer(model, ComponentConfig(name="sgd", params={"lr": 0.1}))
